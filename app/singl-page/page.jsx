// "use client"; qatnashuvini qo'shish muhim
"use client";
...
const ProductPage = () => {
  ...
  const getData = async () => {
    try {
      ...
      if (response.status === 200 && response.data.products) {
        const productData = response.data.products.find(item => item.product_id === id);
  ...
  const handleSubmit = async () => {
    const payload = { productID: id, message: text };
      ...
      <div className="container">
        ...
        <div className="flex justify-between mb-[100px]">
          ...
          <div className="bg-white w-[495px] py-[50px] pr-[80px] rounded-md pl-[50px]">
            <h1 className="text-[32px] mb-4 font-semibold">{data.product_name}</h1>
            <div className="text-[16px]">
              <p className="mb-[10px] font-medium">{data.description}</p>
              <h1 className="text-[16px] inline mr-2">Color:</h1>
              <p className="text-[20px] inline font-semibold">{data?.color?.join(" ")}</p>
              ...
              <p className="text-[24px] mt-[36px] font-semibold">
                {data.cost} <span className="text-[20px] opacity-[0.8]">uzs</span>
              </p>
        ...
        <div className="mb-20 flex gap-5">
          <div>
            ...
            <div className="bg-white max-w-[608px] rounded-lg pt-10 pr-[142px] pb-[116px] pl-[70px]">
              <h1 className="text-[32px] font-semibold">{data.product_name}</h1>
              <div className="text-[16px]">
                <p className="mt-[28px] mb-[53px] font-medium">{data.description}</p>
                ...
                <div className="flex gap-[65px] mt-[30px]">
                  <div>
                    ...
                    <p className="text-[16px]">{data.age_min} - {data.age_max}</p>
                  </div>
                  <div>
          ...
          <div>
            ...
            <div className="bg-white w-[608px] rounded-lg pt-10 pr-[142px] pb-[116px] pl-[70px]">
              {comments.map((item, index) => item.Message && (
                <div key={index}>
                  <h1 className="text-[20px] font-semibold mb-2">{item?.OwnerID}</h1>
